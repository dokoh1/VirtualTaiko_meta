            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                for (int i = 0; i < maxSteps; ++i)
                {
                    ...
                    if (density > 0)
                    {
                        ...
                        float lightFactor = henyey_greenstein(dot(rayDir, mainLight.direction), _LightScattering);
                        fogCol.rgb += mainLight.color.rgb * _LightContribution.rgb * lightFactor * density * mainLight.shadowAttenuation * _StepSize;
